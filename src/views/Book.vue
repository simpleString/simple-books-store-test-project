...
export default {
    ...
    return {
      ...
      rules: {
        ...
        ratingRules: [
          ...
          (v) => v >= 0 || "Value must be greater that 0",
          (v) => v < 11 || "Value must be smaller that 11",
        ],
      },
      valid: true,
      title: "Create",
  ...
  methods: {
    async validate() {
      if (this.$refs.form.validate()) {
        if (this.isEditing) {
          ...
          const result = await this.$store.dispatch("editeBook", this.formData);
    ...
    initEditedData() {
      this.isEditing = true;
      this.title = "Edit";
      const book = this.$store.getters.getBookById(this.$route.params.id);
      this.formData.name = book.name;
      this.formData.authors = book.authors;
      this.formData.public_year = book.public_year;
      this.formData.rating = book.rating;
